...
public class Main {
	...
	/*public List<Course> coursesTaughtByValentine(List<Student> allStudents){
		List<Course> coursesFound = allStudents.stream().filter(a->a.getPlanOfStudy().getCourseList().getPrimaryProfessor().equals("Valentine"))
				.map(a->a.getCourseList()).collect(Collectors.toList());
		return coursesFound;
	}
	...
	public static List<Course> coursesTaughtByValentine(List<Student> allStudents) {
		...
		for(Student s: allStudents){
			List<Course> studentCourseList = s.getPlanOfStudy().getCourseList()
					.stream().filter(c->c.getPrimaryProfessor().equals("Valentine")).collect(Collection.toList());
			if(studentCourseList != null){
				coursesFound.addAll(studentCourseList);
			}
		}
		coursesFound = coursesFound.stream().distinct().collect(Collection.toList());
		return coursesFound;
