...
public class Problem1 {
	...
	public static List<Customer> specialCustomers(List<Account> accts) {
		List<Customer> customerList = accts.stream()
				.filter(a->a.getId().length() == a.getCustomer().getAddress().getCity().length()).map(a->a.getCustomer()).collect(Collectors.toList());
		return customerList;
		//implement
	...
	public static List<Customer> specialAccounts(List<Customer> custs) {
		List<Customer> customerList = custs.stream()
				.filter(c->c.getCheckingAccount().getBalance() < 100 && c->c.getCheckingAccount().getBalance() > 50)
				.filter(c->c.startWith("A"))
				.sort((a,b)->a.getFirstName().compareTo(b.getFirstName())).collect(Collectors.toList());
		return customerList;
